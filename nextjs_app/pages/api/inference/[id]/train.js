...
const train = async (id) => {
  ...
  const neuralNetworkArchitecture = await query({ id, type: "neural_network_architecture" });
  const hiddenLayers = await query({ id, type: "hidden_layers" });
  const neuronsPerHiddenLayer = await query({ id, type: "neurons_per_hidden_layer" });
